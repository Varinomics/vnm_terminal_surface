...
private:
    ...
    void finish_paused_output_delivery(bool delivery_started)
    {
        finish_native_backend_paused_output_delivery(
            ...
            delivery_started,
            [this] {
                // This backend drains while stopping once shutdown has started. The
                // POSIX backend drains through m_process_stopping || m_stopping
                // instead, and wakes its I/O threads when the drain finishes; this
                // backend has no I/O threads to wake. The two stop predicates are
                // not interchangeable and neither is the shared default.
