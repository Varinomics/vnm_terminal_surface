...
status: supported
action_category: screen-mutation
behavior: raster attributes apply only before the first data character or graphics new line, so an image has one aspect ratio; Pan/Pad is rounded up, as OpenConsole does, where the manual says nearest, and an omitted or zero Pad keeps the ratio; P1 above 9 is 1:1; data drawn before a color is selected uses register 15; color numbers past 255 wrap; registers 16 to 255 start with the VT340 map repeated; a register color is taken when a sixel is drawn, while the background takes register 0 as it stands at ST; the extent covers the drawn pixels and, with a background, the declared raster; numeric parameters saturate at 32767; other bytes are ignored; with no cell pixel size known the image is dropped, cursor movement included; an image that would start below the bottom margin is dropped, as OpenConsole drops it; the image is clipped at the right margin, and a band with no drawn pixel leaves its row alone; a cell that receives a drawn pixel loses its text and hyperlink and keeps its style, a wide glyph whole; an image over an earlier image on the same row draws its drawn pixels over the earlier ones, which are first resampled to the new cell pixel size if theirs differs, unless the composite would exceed the decoded-size cap, when the newer image replaces the earlier one on that row and the image gets one DCS sixel payload-limit diagnostic with the largest refused composite size; as in OpenConsole, one sixel row is at most as tall as the scroll region, or the page with DECSDM set, so placement clamps a larger aspect ratio to (rows x cell height) / 6: each sixel pixel keeps the rows the clamped ratio gives it and the final sixel row and scrolling follow the clamped ratio, but a background raster declared taller than the drawn sixels shrinks in the same proportion, and an image the unclamped ratio took over the decoded-size cap keeps no pixels; scrolls past an image's pixels stop once the region has scrolled its full height, which leaves the screen and cursor as the full count would and adds at most one region of blank rows to history; an image starting above the top margin keeps the bands drawn above the region where they are and scrolls only the region; each text row keeps at most one image, which moves and dies with the row and reaches history with it, and a row whose history record would exceed the record limit (dcs-sixel-decoded-limit) keeps its text and drops its image, at append and when the ring shrinks; printing into a cell and erasing it with EL, ECH or ED 0 and 1 clear the image pixels of that cell, text or not, and a row image left with no drawn pixel is dropped; ED 2 drops the images of the screen; ICH and DCH move image columns with the cells, losing those pushed past the right margin or deleted; on the screen, placing an image hard-terminates the soft wraps into and out of the rows it covers, and later text wrapping into or out of a row that shows an image wraps hard, so each image row stays independent of the adjacent logical lines; the one wrap left as recorded is the one from the newest history row into the top screen row when an image lands there, which nothing reads, as history is never reflowed and selection and search do not join rows across it; reflow keeps a row image on the first row of its logical line, its columns never make continuation rows, and widening back restores it unchanged; a repaint that recovers rows into history keeps the images those rows showed; provisional, for image columns left past the right margin by a narrowing: EL, ECH and ED 0 and 1 clear only the cells on the grid, so those columns show again after widening, ED 2 drops the whole row image, and ICH and DCH drop them, so ICH or DCH left of an image lying wholly past the margin drops it
host_policy: none
